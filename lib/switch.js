...
function whois(hashname)
{
  ...
  ret.start = function(type, arg, callback)
  {
    ...
    chan.handler = function(packet, cbHandler)
    {
      ...
      // the app should call chan.setup() that reconfigures chan.handler
...
function channel(type, id)
{
  ...
  chan.end = function(err){chan.ack(err||true)};
    ...
    if(chan.ended) return warn("can't send to an ended channel");
	  ...
	  if(!end && !arg && chan.outConfirmed == chan.inSeq && !chan.inDups) return;
    ...
    if(chan.outSeq === 0) packet.js.type = (chan.app?"_":"")+chan.type;
