...
// incoming channels
me.start(type, function(arg, callback){
  callback(err, arg, handler); // handler is optional, returns chan
})

// outgoing channels
you.start(type, arg, function(err, arg, callback){
  callback(err, handler); // this is optional, not for ask, returns chan
})

var startChannels = {
	"ask":function(err, arg){
    if(arg && arg.js) arg.js = {ask:arg.js};
    // asks always end immediately
		this.ack(err||true,arg);
	},
	"packet":function(err, arg, cbPacket){
    this.ack(err, arg);
    if(err) return;
    this.packet = this.send; // to send raw packets
    this.end = function(err){this.ack(err||true)};
		this.receive = function(packet){
      cbPacket(packet.js.err||packet.js.end, packet);
    }
	},
	"message":function(err, arg, cbMessage){
    if(arg && arg.js) arg.js = {message:arg.js};
    this.ack(err, arg);
    if(err) return;
    this.end = function(err){this.ack(err||true)};
    // to send new messages
		this.message = function(arg, cbConfirm){
      arg.js = {message:arg.js};
      arg.callback = cbConfirm;
      this.ack(false, arg);
    };
		this.receive = durable; // calls this.handle
    this.handle = function(packet, callback){
	...
	"stream":function(err, arg, cbRead){
    if(arg && arg.js) arg.js = {stream:arg.js};
    this.ack(err, arg);
    if(err) return;
    this.end = function(err){this.ack(err||true)};
    this.write = function(data, cbMore)
    {
      // break data into chunks
      // if outgoing is full, this.more = cbMore
    }
    this.receive = durable;
    this.handle = function(packet, callback)
    {
      // TODO if this.more and outgoing isn't full, var more=this.more;delete this.more;more()
      if(!packet.body && !packet.js.end) return callback(); // odd empty?
      cbRead(packet.js.err||packet.js.end, packet.body, callback);
    }
	},
	"bulk":function(err, arg, cbBulk){
    if(arg && arg.js) arg.js = {bulk:arg.js};
    this.ack(err, arg);
    if(err) return;
    this.receive = durable;
    var bulk = "";
...
{
	if(type != type != "message" || type != "bulk" || type != "packet") return warn("invalid type given, must be message, bulk, or packet");
  self.listens[type] = callback;
}
...
function whois(hashname)
{
  ...
  ret.at = Date.now();
	ret.channel = makeChannel;
	// convenience wrapper to start a new channel to this hashname
	ret.ask = function(type, js, callback)
	{
		if(!type) return callback("invalid args to connect()");
		// detect any custom types and make sure to _escape them
		if(!self.listening[type] && type.substr(0,1) != "_") type = "_"+type;
		var chan = hn.channel(type);
		var packet = chan.packet(js);
		chan.send(packet);
		chan.onAnswer = callback;
	}
}

function chan
function makeChannel(type, id)
{
	var hn = this;
  ...
  chan.hashname = hn; // for convenience
	...
	// called as the first thing to answer a new channel
	chan.answer = function(err, js, body, callback)
	{
		// if err is true, just end it, if a string attach
		// set up .handle to be depending on the type
		// callback args depend on the type
	}

	// wrapper for apps to send/receive individual messages, ordered/reliable
	chan.message = function(js, body, callback)
	{
		if(chan.type !== "message") return warn("can't call .message on a non-message channel");
		chan.send(chan.packet(js, body), callback);
	}
	
	// app interface to send bulk/blob large data
	chan.bulk = function(data, callback)
	{
		// iterate over data sending packets, implement backpressure, end when done
	}
