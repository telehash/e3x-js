...
hn.start(type, arg, callback)
  ...
  - callback(err, arg, cbChannel)
    - called when a response is received (or error/fail)
    - given the response .js .body in arg
    - for ask type, cbChannel isn't given
    - to finish creating channel, call cbChannel(err, arg, handler);
      - returns chan object
      - err ends the channel
      - arg depends on type, .bulk
      - handler depends on type, called for all activity
self.start(type, callback)
  - used to listen for incoming channel starts
  - callback(arg, cbChannel)
    - called for any incoming channel start
    - to answer, call cbChannel(err, arg, handler)
      - err to end/fail, arg contains .js .body response
      - arg also depends on type, .bulk
      - returns working channel (if not an err)
      - handler optional depends on type, called for all activity

TYPE-DEPENDENT functions on channels
chan.packet() - raw
chan.message() - ad-hoc anytime
chan.write() - for streaming
bulk is given in arg.bulk during init
...
function receive(msg, from)
{
  ...
  if(packet.js.type == "open")
    ...
    // line is open now!
      ...
      // TODO find channel or create
      // create: use .listening
      // if found, .receive
  	}
...
function whois(hashname)
{
  ...
  // trying to send on a line needs to create it first
