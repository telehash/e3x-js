...
function whois(hashname)
{
  ...
  hn.send = function(packet){
    if(!hn.lineQ) hn.lineQ = [];
    // skip duplicate packets
    for(var i=0;i<hn.lineQ.length;i++)
    {
      if(hn.lineQ[i].id == packet.id) return;
    }
    // queue them up to be sent after a line is created
    hn.lineQ.push(packet);
  ...
  hn.seek = function(hashname, callback)
  {
    ...
    chan.receive = function(packet){
      callback(packet.js.err,packet.js.see);
    }
  }
