...
exports._self = function(args){
  ...
  Object.keys(csets).forEach(function(csid){
    ...
    var promise = new csets[csid]._Local(args.pairs[csid])
    localPromises.push(
      promise.then(function(local){
        self.locals[csid] = local
      }).catch(function(er){
        console.log("ER")
        exports.err = er;
      })
  ...
  return Promise.all(localPromises).then(function(){
    self.decrypt = function e3x_decrypt(message){
      if(typeof message != 'object' || !Buffer
      .isBuffer(message.body) || message.head.length != 1) return false;
      var csid = message.head.toString('hex');
      if(!self.locals[csid]) return false;
      return self.locals[csid].decrypt(message.body)
                              .then(lob.decode);
    }

    self.exchange = function e3x_exchange(args){
      ...
      var makeRemote = new csets[csid]._Remote(key);

      return makeRemote.then(function(remote){
        var cs = remote;
        ...
        x.encrypt = function(inner){
          return cs.encrypt(self.locals[csid], inner)
                   .then(lob.packet.bind(null,csid1));
        };
        ...
        x.receive = function(packet){
          ...
          x.session.decrypt(packet.body.slice(16))
                   .then(lob.decode)
                   .catch(x.error);
        };
        ...
        function makeSession (x, cs, handshake_body ){
          var makeSession = new csets[csid].Ephemeral(cs, handshake_body);

          return makeSession.then(function(session){
            ...
            return x.error('session error: '+session.err);
          })
        }

        x.sync = function(handshake, inner){
          if(!handshake) return false;
          inner = (inner) ? Promise.resolve(inner)
                          : self.decrypt(handshake); // optimization to pass one in already done
          return inner.then(x.verify.bind(x, handshake))
                      .then(function(ver){
                        if (!ver) throw new Error("handshake failed to verify")
                        // create/update session if needed
                        var sid = handshake.slice(0,16).toString('hex'); // stable token bytes

                        return (x.sid != sid) ? makeSession(x, cs, handshake.body) : Promise.resolve();
